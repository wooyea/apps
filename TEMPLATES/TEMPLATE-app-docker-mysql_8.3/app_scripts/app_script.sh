...
app_init() {
    ...
    [ -d ${APP_DIR}/var/mysql-files ] || su_run "mkdir -p ${APP_DIR}/var/mysql-files" 
    ...
    docker create --net=host                    \
        ...
        -v ${APP_DIR}/var/mysql-files:/var/lib/mysql-files    \
...
app_client() {
    docker exec -it $DOCKER_CONTAINER_NAME  bash -c 'export TERM=xterm; mysql --defaults-file=/etc/my.cnf -S /var/run/mysqld/mysqld.sock -u root -p'
}
